...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) 
{
	...
	// your code here
  	int   rows, cols, new_r, new_c;
  	float center_prob, corner_prob, adjacent_prob, grid_val, mult;
    rows = grid.size();
    cols = grid[0].size();
      ...
      for (int c = 0; c < cols; c++)
      {
        ...
        for(int dx = -1; dx < 2; dx++)
        {
          for(int dy = -1; dy < 2; dy++)
          {
            ...
            new_r = ((r+dy) % rows + rows)%rows;
            new_c = ((c+dx) % cols + cols)%cols;
            newGrid[new_r][new_c] += mult * grid_val;
          }  
        }
      }
    }
